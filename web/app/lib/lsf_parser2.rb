...
class LSFParser2
  ...
  def self.find_suitable_urls(link = TOPLEVEL)
    LSFParser2.broadcast "Now checking #{link}"

    dec = LSFParser2.load_url(link)
  ...
  def self.find_certain_roots(search)
    ...
    urls = LSFParser2.find_suitable_urls
...
# tree-walk. Also returns them as term, rootid.
  ...
  # completes professor information for the given ID and name. Name must
    ...
    def self.get_prof(id)
      ...
      LSFParser2.broadcast "Reading Professor Details: #{id}"
      root = LSFParser2.load_xml("getDet?pid=#{id}")
      root.remove_namespaces!
      root2 = LSFParser2.load_xml("getKontakt?pid=#{id}")
...
def self.get_profs(id)
  root = LSFParser2.load_xml('getDozenten?vtid=' + id.to_s)
  profs = root.xpath("//ns:return").flat_map { |p| LSFParser2.get_prof(p.at_xpath("//ax23:id").content()) }
  ...
  def self.get_lecture_details(id)
    LSFParser2.broadcast("Reading lecture details: #{id}")
    root = LSFParser2.load_xml("getVerDet?vid=#{id}")
    veranst = nil
    veranst = Course.find_by(title:root.at_xpath("//ax23:dtxt").content()) unless root.at_xpath("//ax23:dtxt").nil?
    unless veranst.nil?
      termine = LSFParser2.load_xml("getTermine?vid=#{id}")
      raume= termine.xpath("//ns:return").flat_map do |ldat|
        {:tag=>ldat.at_xpath("//ax23:wochentag").content(),
          :beginn=> ldat.at_xpath("//ax23:beginn").content()
        }
      end
      veranst.description = raume.map{|r| r[:tag]+":"+r[:beginn]}.join ";"
      veranst.save
  end
  end
  ...
  def self.get_lecture(id)
    ...
    LSFParser2.broadcast("Reading lecture: #{id}")
    @@level += 1
    l = LSFParser2.get_lecture_details(id)
=begin
    skip = !l.nil?
    skip = !l[:type].nil?
...
=end
  ...
  def self.get_lectures(id)
    LSFParser2.broadcast "Reading Lecture List #{id}"
    @@level += 1
    root = LSFParser2.load_xml("getVorl?ueid=#{id}")
    l = root.xpath("//ns:return").flat_map do |ldat|
      lect = LSFParser2.get_lecture(ldat.at_xpath("ax23:vorID").content())
    end.compact
    @@level -= 1
    l
  end
  # Takes a "root id" as argument and recursively parses the LSFParser2 tree
# down to the event level
def self.get_tree(id, processed_trees = [])
  LSFParser2.broadcast "Loading Tree: #{id}"
  ...
  if processed_trees.include?(id)
    LSFParser2.broadcast "Tree #{id} has already been covered, skipping"
    return []
  end
  processed_trees << id

  root = LSFParser2.load_xml("getUeberschr?ueid=#{id}&semester=#{LSFParser2.term}")
  lectures = []
  # If the branch has sub branches, then read them
  LSFParser2.broadcast root.xpath("//ns:return").size.to_s
  #o = gets.chomp
  if (root.elements.size >= 1)&&(root.elements.first.elements.size>=1)
    root.xpath("//ns:return").flat_map do |subtree|
      ...
      next if subid.nil?
      LSFParser2.broadcast "Reading Subtree: #{subid.content()}"
      lectures += LSFParser2.get_tree(subid.content(), processed_trees)
    end
  # Otherwise get list of lectures in that leaf.
  else
    LSFParser2.broadcast "Found leaf at #{id}. Reading lectures."
    #o= gets.chomp
    lectures += LSFParser2.get_lectures(id)
  end
...
# returns the term as either set manually or guessed.
  ...
  def self.load_xml(parameters)
    ...
    LSFParser2.broadcast "Lade "+url
    @@cache_xml[url]=Nokogiri::XML(LSFParser2.load_url(url))
    @@cache_xml[url]
  end
  def self.process(courses, title,semester,invite)
    ...
    courses.each do |course|
      LSFParser2.broadcast "Importiere #{i}/#{courses.size}"
      i+=1
        ...
        course[:profs].each do |prof|
          ...
          if pr.nil?
            ...
            if invite
              ...
              LSFParser2.broadcast "Sende Email"
            end
          end
  ...
  def self.import(names,invite=false)
    ...
    LSFParser2.broadcast("Importvorgang erfolgreich beendet",false);
  end
